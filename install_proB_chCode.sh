...
export chaincodeVersion=$name+"_"+$version
export chaincodeName="proBlood.tar.gz"
export chaincodePath="../bloodPro-chain/"
...
peer chaincode invoke -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem" -C mychannel -n blood --peerAddresses localhost:7051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org1.example.com/peers/peer0.org1.example.com/tls/ca.crt" --peerAddresses localhost:9051 --tlsRootCertFiles "${PWD}/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls/ca.crt" -c '{"function":"createBloodProfile","Args":["dor001","proB001","1","2","3","4","5","6","7","8","9"]}'

echo "==================================== query ===================================="

peer chaincode query -C mychannel -n blood -c '{"Args":["queryProfile", "proB001"]}'
